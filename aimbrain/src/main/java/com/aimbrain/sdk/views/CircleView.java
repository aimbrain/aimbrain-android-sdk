...
public class CircleView extends View {
    ...
    //draws circle progress when recording
    private Paint mCirclePaint;
    //draws grey circle when not recording
    private Paint mGreyPaint;
    //draws whole circle when recording ( this should be lighter color
    private Paint mCircleInactivePaint;
    ...
    public CircleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int circleInactiveColor = Color.GRAY;
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.CircleView);
            if (ta != null) {
                circleColor = ta.getColor(R.styleable.CircleView_circleColor, circleColor);
                circleInactiveColor = ta.getColor(R.styleable.CircleView_circleColorInactive,
                        circleInactiveColor);
                ta.recycle();
            }
        }
        ...
        mCircleInactivePaint = new Paint();
        mCircleInactivePaint.setAntiAlias(true);
        mCircleInactivePaint.setColor(circleInactiveColor);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            greyColor = context.getResources().getColor(R.color.color_circle_inactive, context.getTheme());
        } else {
            greyColor = context.getResources().getColor(R.color.color_circle_inactive);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCircleSweepAngle > 0f) {
            mCanvas.drawOval(mCircleOuterBounds, mCircleInactivePaint);
            mCanvas.drawArc(mCircleOuterBounds, ARC_START_ANGLE, mCircleSweepAngle, true, mCirclePaint);
            mCanvas.drawOval(mCircleInnerBounds, mEraserPaint);

        } else {
            mCanvas.drawOval(mCircleOuterBounds, mGreyPaint);
            mCanvas.drawOval(mCircleInnerBounds, mEraserPaint);
        }
